...
| RF18 | Segunda visão da tela de Histórico, organizada por dia (não por exercício): sessões finalizadas agrupadas por dia civil e, dentro de cada dia, por sessão (treino + exercícios com séries, na unidade correta da categoria — RF17); alternância "Por exercício" / "Por data" na mesma tela, sem navegação (estende o RF08/RF17) | Média | `specs/019-historico-por-data/` |
| RF19 | Catálogo interno de exercícios (nome, grupo muscular, imagem), 30 itens curados a partir do wger project (CC-BY-SA), embutido no app, 100% offline; sem UI própria de navegação nesta etapa; tela de créditos/atribuição (exigência da licença) | Baixa | `specs/020-catalogo-exercicios/` |
