...
- [ ] T001 Confirmar que o development build usado para validar o RF07 continua instalado e funcional nos dois aparelhos-alvo (Redmi Note 12 e iPhone 16 Plus) — esta feature não introduz nenhum módulo nativo novo, mas continua exigindo o mesmo build (não Expo Go), pelo motivo já registrado em plan.md, "Testing" (import incondicional de `expo-notifications` em `src/app/_layout.tsx` desde o RF06). **Pendente do usuário** — esta sessão de implementação não tem acesso aos aparelhos físicos; toda a validação funcional abaixo foi feita via `expo start --web` + Chromium headless como evidência substituta, não como confirmação do Princípio III.
...
- [ ] T019 Rodar a validação completa de [quickstart.md](./quickstart.md) nos dois aparelhos-alvo (Redmi Note 12/Android e iPhone 16 Plus/iOS), conforme Constituição Princípio III, cobrindo todos os 6 cenários de ponta a ponta antes de considerar o RF08 concluído. **Pendente do usuário.** Todos os 6 cenários já foram exercitados funcionalmente via `expo start --web` nesta sessão (T009, T012, T014, T015), sem nenhum erro de console/página — falta apenas a confirmação formal em Android e iOS físicos, exigida pelo Princípio III, que este ambiente de implementação não tem como fazer.
